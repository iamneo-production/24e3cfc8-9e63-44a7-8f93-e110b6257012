...
export default function LoginPage(props) {
    ...
    const handleLogin = async (e) =>{
        e.preventDefault();
        console.log(data);
        axios.post('https://8080-bbfbddbccbeeeffafbbcefaabfececdfccc.project.examly.io/login',data)
        .then(response =>{
        console.log(`${response.data} from login`)
        if(response.data===true)
            {
                axios.get(`https://8080-bbfbddbccbeeeffafbbcefaabfececdfccc.project.examly.io/getuser/${data.email}`).then(response=>
                {
